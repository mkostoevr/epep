...
int main(int argc, char **argv) {
	struct {
		int c; // COFF File Header
		int o; // Optional Header
		int d; // Data Directories
		int s; // Sections
		int S; // Symbols
		int i; // Imports
		int e; // Exports
		int b; // Base Relocations
		int r; // COFF Relocations
		int l; // Linenumbers
	} flags = { 0 };
	...
	if (argc < 2) {
		...
		char *flags_str = argv[1];
		for (int i = 0; i < strlen(flags_str); i++) {
			switch (flags_str[i]) {
	...
	if (flags.i && epep_has_import_table(&epep)) {
	...
	if (flags.e && epep_has_export_table(&epep)) {
	...
	if (flags.b && epep_has_base_relocation_table(&epep)) {
	...
	if (flags.r && epep.kind == EPEP_OBJECT) {
		for (size_t i = 0; i < epep.coffFileHeader.NumberOfSections; i++) {
			EpepSectionHeader sh = { 0 };
			if (!epep_get_section_header_by_index(&epep, &sh, i)) {
				return ERROR(epep);
	...
	if (flags.l && epep.kind == EPEP_OBJECT) {
		for (size_t i = 0; i < epep.coffFileHeader.NumberOfSections; i++) {
			EpepSectionHeader sh = { 0 };
			if (!epep_get_section_header_by_index(&epep, &sh, i)) {
				return ERROR(epep);
